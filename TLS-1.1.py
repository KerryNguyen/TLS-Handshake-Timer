import os, ssl, pyfiglet, socket, warnings, time

os.system('cls')

print(pyfiglet.figlet_format("TLS-1.1", font="smblock"))

# Used to suppress deprecation warnings, as this will fill up the screen.
...
# openssl s_server -accept 6768 -cert cert.pem -key key.pem -min_protocol TLSv1.1 -cipher "ALL:@SECLEVEL=0
# raw_sock = socket.create_connection(("localhost", port))

# Setting up the settings for the client
# We're setting it to ONLY use TLS 1.1
ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
ctx.minimum_version = ssl.TLSVersion.TLSv1_1
ctx.maximum_version = ssl.TLSVersion.TLSv1_1

# This should unblock the old TLS 1.1 ciphers and disable security checks
# Disable Certificate Checking since our Certificate is self-signed and would FAIL
...
print(f"File has been saved to:", os.path.join(os.getcwd(), timesheet))
print(f"\nResults:")
